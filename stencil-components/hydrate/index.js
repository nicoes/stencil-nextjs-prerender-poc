...
const paths$1 = {
  'menu': () => hAsync("path", { d: "M1.1.8h25v2h-25zM1.1 10.1h25v2h-25zM1.1 19.4h18.1v2H1.1z" }),
  'close': () => hAsync("path", { d: "M21.999.195l1.843 1.844-9.677 9.675 9.677 9.678-1.843 1.843-9.677-9.677-9.677 9.677-1.843-1.843 9.677-9.677L.802 2.04 2.645.195l9.677 9.676L21.999.195z" })
};
const Icon$1 = (path) => ({ className }) => hAsync("svg", { class: className, width: "24", height: "24", fill: "#323232", xmlns: "http://www.w3.org/2000/svg" }, path());
const Icons$1 = {
  'menu': Icon$1(paths$1.menu),
  'close': Icon$1(paths$1.close)
};

const AnalyticsDataAttributes$1 = ({ action, label }) => ({
  'data-analytics-action': action,
  'data-analytics-label': label
});

const ProfileIcon = ({ withFill, className }) => hAsync("svg", { class: className, width: "20", height: "20", fill: "none", xmlns: "http://www.w3.org/2000/svg" },
  hAsync("circle", { cx: "10", cy: "10", r: "9.5", stroke: "#232D3D" }),
  hAsync("mask", { id: "a", style: { "mask-type": "alpha" }, maskUnits: "userSpaceOnUse", x: "0", y: "0", width: "20", height: "20" },
    hAsync("circle", { cx: "10", cy: "10", r: "10", fill: "#232D3D" })),
  hAsync("g", { mask: "url(#a)" },
    hAsync("path", { d: "M3.757 17.161a9.504 9.504 0 0 1 2.535-1.588A6.169 6.169 0 0 0 10 16.823a6.169 6.169 0 0 0 3.708-1.25c.93.395 1.784.933 2.535 1.588A9.462 9.462 0 0 1 10 19.5a9.462 9.462 0 0 1-6.243-2.339Z", fill: withFill ? "#232D3D" : '', stroke: "#232D3D" })),
  hAsync("path", { d: "M10 3.792c2.029 0 3.875 2.06 3.875 4.875 0 1.307-.668 2.536-1.546 3.458-.436.457-.906.82-1.338 1.066-.446.255-.793.35-.991.35-.198 0-.545-.095-.991-.35a6.008 6.008 0 0 1-1.338-1.066c-.878-.922-1.546-2.15-1.546-3.458 0-2.816 1.846-4.875 3.875-4.875Z", fill: withFill ? "#232D3D" : '', stroke: "#232D3D", "stroke-width": "1.25" }));

const accountUnauthenticatedListItems = [
  {
...
const accountAuthenticatedListItems = [
  {
    value: '/account',
    label: 'Account'
  },
...
const getIconForLabel = (label) => {
  if (label === 'Inloggen') {
    return hAsync(ProfileIcon, { className: 'profile-icon' });
  }
  else if (label === 'Account') {
    return hAsync(ProfileIcon, { className: 'profile-icon', withFill: true });
  }
};
const TopBar = ({ emailAddress }) => hAsync("div", { class: 'top' },
  hAsync("ul", { class: 'top-list' },
    !!emailAddress && accountAuthenticatedListItems.map(item => {
      return hAsync("li", null,
        hAsync("a", Object.assign({ href: item.value }, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: item.analyticsLabel || item.label })),
          getIconForLabel(item.label),
          item.label === 'Account' ? emailAddress : item.label));
    }),
  ...
  hAsync("a", Object.assign({ href: "/werkgever", class: "top__employer-button" }, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderWerkgeversLink', label: 'VoorWerkgevers' })), "Voor werkgevers"));

const createActiveClassExtender = (baseClass, activeClass, document) => (startMatch) => {
  var _a;
  const pathname = (_a = document === null || document === void 0 ? void 0 : document.location) === null || _a === void 0 ? void 0 : _a.pathname;
  if (pathname) {
...
const AnalyticsDataAttributes = ({ action, label }) => ({
  'data-analytics-action': action,
  'data-analytics-label': label
});

const AnchorList$1 = ({ listItems, analyticsAction, className }) => listItems.map(item => hAsync("li", null,
  hAsync("a", Object.assign({ class: className || '' }, AnalyticsDataAttributes({ action: analyticsAction, label: item.analyticsLabel || item.label }), { href: item.alternateDomain ? item.alternateDomain + item.value : item.value }), item.label)));

const DropdownContainer$1 = ({ listItems, analyticsAction }) => hAsync("ul", { class: 'dropdown-list' },
  hAsync(AnchorList$1, { listItems: listItems, analyticsAction: analyticsAction }));

const topicsListItems = [
  {
    label: 'Werk en carrière',
    value: "/werk-en-carriere",
    analyticsLabel: 'WerkEnCarrière'
  },
  {
    label: 'Werk-privébalans',
    value: "/werk-privebalans",
    analyticsLabel: 'WerkPrivébalans'
  },
  {
    label: 'Personal finance',
    value: "/personal-finance",
    analyticsLabel: 'PersonalFinance'
  },
  {
    label: 'Persoonlijke groei',
    value: "/persoonlijke-groei",
    analyticsLabel: 'PersoonlijkeGroei'
  },
  {
    label: 'Emancipatie en diversiteit',
    value: "/emancipatie-en-diversiteit",
    analyticsLabel: 'EmancipatieEnDiversiteit'
  },
  {
    label: 'Zzp',
    value: "/zzp"
...
const MainMenuLink = ({ item, withDropdown }) => hAsync("a", Object.assign({ href: item.value }, AnalyticsDataAttributes({ action: 'ClickOnHeaderLink', label: item.analyticsLabel || item.label }), { class: 'menu-link' + (withDropdown ? ' menu-link--dropdown' : '') }), item.label);

const jobsListItems$1 = [
  {
    label: 'Zoeken',
    value: "/vacature/zoeken",
...
const AnchorList = ({ listItems, analyticsAction, className }) => listItems.map(item => hAsync("li", null,
  hAsync("a", Object.assign({ class: className || '' }, AnalyticsDataAttributes$1({ action: analyticsAction, label: item.analyticsLabel || item.label }), { href: item.value }), item.label)));

const SubMenu$1 = ({ listItems, analyticsAction }) => hAsync("div", { class: 'sub-menu' },
  hAsync("ul", { class: 'sub-menu__inner' },
    hAsync(AnchorList, { listItems: listItems, analyticsAction: analyticsAction })));

const cityListItems$1 = [
  {
    label: 'Amsterdam',
...
const accountAuthenticatedMobileListItems = [
  ...
  },
  {
    ...
    label: 'Jouw mailinstellingen',
    analyticsLabel: 'JouwMailinstellingen'
  },
  {
    value: '/account',
    label: 'Jouw account',
    analyticsLabel: 'JouwAccount'
  },
  {
    value: '/account/uitloggen',
    label: 'Uitloggen'
...
const MobileMenuAccountAuthenticated$3 = () => hAsync("ul", { class: 'mobile-menu__link-list mobile-menu__link-list--account' }, accountAuthenticatedMobileListItems.map(item => {
  return hAsync("li", null,
    hAsync("a", Object.assign({ href: item.value }, AnalyticsDataAttributes({ action: 'ClickOnHeaderAccountLink', label: item.analyticsLabel || item.label })), item.label));
}));

const MobileMenuAccountAuthenticated$2 = () => hAsync("ul", { class: 'mobile-menu__link-list mobile-menu__link-list--account' },
  hAsync("li", null,
    hAsync("a", Object.assign({}, AnalyticsDataAttributes({ action: 'ClickOnHeaderAccountLink', label: 'Inloggen' }), { href: '/account/inloggen' }), "Inloggen")),
  hAsync("li", null,
    hAsync("a", Object.assign({}, AnalyticsDataAttributes({ action: 'ClickOnHeaderProfileLink', label: 'ProfielMaken' }), { href: '/profiel/registreren' }), "Profiel maken")),
  hAsync("li", null,
    hAsync("a", Object.assign({}, AnalyticsDataAttributes({ action: 'ClickOnHeaderProfileLink', label: 'Adverteren' }), { href: '/profiel/registreren' }), "Adverteren")));

const MobileMainMenuLink = ({ item, withDropdown }) => hAsync("a", Object.assign({ href: item.value }, AnalyticsDataAttributes({ action: 'ClickOnHeaderLink', label: item.analyticsLabel || item.label }), { class: 'mobile-menu__link' + (withDropdown ? ' mobile-menu__link--dropdown' : '') }), item.label);

var CollapsibleId;
(function (CollapsibleId) {
  CollapsibleId[CollapsibleId["JOBS"] = 0] = "JOBS";
  CollapsibleId[CollapsibleId["TOPICS"] = 1] = "TOPICS";
  CollapsibleId[CollapsibleId["EDUCATION"] = 2] = "EDUCATION";
})(CollapsibleId || (CollapsibleId = {}));
const MobileMenu$1 = ({ isVisible, handleModalClose, isAuthenticated, isOpenTopicsMenu, isOpenEducationMenu, isOpenJobsMenu, handleToggleCollapsible }) => {
      ...
      hAsync("div", { class: "mobile-menu__header" },
        ...
        hAsync("div", { class: "mobile-menu__header-button mobile-menu__header-button--close" },
          hAsync("button", { onClick: () => handleModalClose(), class: "mobile-menu__close-button" },
            hAsync(Icons$1.close, { className: 'mobile-menu__close-button-icon' })))),
      hAsync("div", { class: 'mobile-menu__main' },
        hAsync("ul", { class: 'mobile-menu__link-list' },
          hAsync("li", null,
            hAsync(CollapsibleMenu, { isOpen: isOpenTopicsMenu, onClickToggle: () => handleToggleCollapsible(CollapsibleId.TOPICS), analyticsAction: 'ClickOnHeaderTopicsLink', topLinkItem: mainMenuLinks.TOPICS, linkList: topicsListItems })),
          hAsync("li", null,
            hAsync(MobileMainMenuLink, { item: mainMenuLinks.TESTEN_EN_TOOLS })),
          hAsync("li", null,
            hAsync(CollapsibleMenu, { isOpen: isOpenEducationMenu, onClickToggle: () => handleToggleCollapsible(CollapsibleId.EDUCATION), analyticsAction: 'ClickOnHeaderOpleidingenLink', topLinkItem: mainMenuLinks.OPLEIDINGEN, linkList: educationItems })),
          hAsync("li", null,
            hAsync(CollapsibleMenu, { isOpen: isOpenJobsMenu, onClickToggle: () => handleToggleCollapsible(CollapsibleId.JOBS), analyticsAction: 'ClickOnHeaderWerkZoekendenLink', topLinkItem: mainMenuLinks.VACATURES, linkList: jobsListItems$1 }))),
        isAuthenticated && hAsync(MobileMenuAccountAuthenticated$3, null),
        !isAuthenticated && hAsync(MobileMenuAccountAuthenticated$2, null)),
      !isAuthenticated && hAsync("div", { class: "mobile-menu__employer-button" },
        hAsync("a", Object.assign({ href: "/werkgever" }, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderWerkgeversLink', label: 'VoorWerkgevers' })), "Voor werkgevers"))));
};

const iolHeaderB2cCss = "/*!@header*/header.sc-iol-header-b2c{all:initial;font-family:co-text, Helvetica, Arial, sans-serif;font-weight:400;display:flex;flex-direction:column;align-items:stretch}/*!@a*/a.sc-iol-header-b2c{text-decoration:none}/*!@a:hover*/a.sc-iol-header-b2c:hover{text-decoration-line:underline}@supports (contain: none){/*!@a:focus-visible*/a.sc-iol-header-b2c:focus-visible{outline:none;outline:1px solid #09181b}}@supports not (contain: none){/*!@a:focus*/a.sc-iol-header-b2c:focus{outline:none;outline:1px solid #09181b}}/*!@button*/button.sc-iol-header-b2c{font-family:inherit;font-size:100%;line-height:1.15;margin:0;overflow:visible;text-transform:none;-webkit-appearance:button;-webkit-appearance:none;background:none;border:0;cursor:pointer;padding:0;text-decoration:none;box-sizing:content-box}/*!@button:hover*/button.sc-iol-header-b2c:hover{text-decoration-line:underline}/*!@.container*/.container.sc-iol-header-b2c{box-sizing:border-box;max-width:75rem;width:100%;margin-left:auto;margin-right:auto;padding-left:0.5rem;padding-right:0.5rem}@media (min-width: 992px){/*!@.container*/.container.sc-iol-header-b2c{padding-left:1.375rem;padding-right:1.375rem}}/*!@.container--top*/.container--top.sc-iol-header-b2c{display:none}@media (min-width: 992px){/*!@.container--top*/.container--top.sc-iol-header-b2c{display:block}}/*!@.top*/.top.sc-iol-header-b2c{display:flex;justify-content:flex-end;align-items:center}/*!@.top a*/.top.sc-iol-header-b2c a.sc-iol-header-b2c{color:#3a424f;font-size:0.875rem}/*!@.top-list*/.top-list.sc-iol-header-b2c{list-style:none;padding:0;margin:0;display:flex;height:2.5rem}/*!@.top-list li*/.top-list.sc-iol-header-b2c li.sc-iol-header-b2c{display:flex;flex:1;justify-content:center;align-items:center}/*!@.top-list li + li:before*/.top-list.sc-iol-header-b2c li.sc-iol-header-b2c+li.sc-iol-header-b2c:before{content:\" \";border-left:1px solid #7c868e;margin-left:0.5rem;margin-right:0.5rem;height:1rem}/*!@.top-list a*/.top-list.sc-iol-header-b2c a.sc-iol-header-b2c{display:flex;flex:1;align-items:center;white-space:nowrap}/*!@.top__employer-button*/.top__employer-button.sc-iol-header-b2c{display:flex;height:2.5rem;align-items:center;justify-content:center;padding-left:1rem;padding-right:1rem;background:#B2DBBF;margin-left:1rem;transition:background-color 0.3s ease}/*!@.top__employer-button:hover*/.top__employer-button.sc-iol-header-b2c:hover{text-decoration:none;background:#99C8A3}/*!@.profile-icon*/.profile-icon.sc-iol-header-b2c{margin-right:0.25rem}/*!@.main*/.main.sc-iol-header-b2c{background-color:#232D3D}/*!@.main-inner*/.main-inner.sc-iol-header-b2c{display:flex;flex-direction:row;justify-content:space-between;align-items:stretch}@media (min-width: 992px){/*!@.main-inner*/.main-inner.sc-iol-header-b2c{justify-content:flex-start}}/*!@.logo*/.logo.sc-iol-header-b2c{display:flex;height:3.75rem;align-items:center;margin-left:0.5rem}/*!@.dropdown-list*/.dropdown-list.sc-iol-header-b2c{list-style:none;padding:0;margin:0;background:#F4E956;box-shadow:0 1px 8px 0 rgba(35, 45, 61, 0.3);top:3.75rem;left:0;visibility:hidden;z-index:999;position:absolute}/*!@.dropdown-list a,\n.dropdown-list button,\n.menu-link*/.dropdown-list.sc-iol-header-b2c a.sc-iol-header-b2c,.dropdown-list.sc-iol-header-b2c button.sc-iol-header-b2c,.menu-link.sc-iol-header-b2c{text-transform:uppercase;font-family:co-headline, Helvetica, Arial, sans-serif;font-weight:700;white-space:nowrap;font-size:12px;letter-spacing:1.33px}/*!@.dropdown-list a:hover,\n.dropdown-list button:hover,\n.menu-link:hover*/.dropdown-list.sc-iol-header-b2c a.sc-iol-header-b2c:hover,.dropdown-list.sc-iol-header-b2c button.sc-iol-header-b2c:hover,.menu-link.sc-iol-header-b2c:hover{text-decoration:none}/*!@.dropdown-list a,\n.dropdown-list button*/.dropdown-list.sc-iol-header-b2c a.sc-iol-header-b2c,.dropdown-list.sc-iol-header-b2c button.sc-iol-header-b2c{display:block;line-height:2.75rem;text-align:left;color:#232D3D;box-sizing:border-box;padding-left:1rem;padding-right:1rem}/*!@.dropdown-list a:hover,\n.dropdown-list button:hover*/.dropdown-list.sc-iol-header-b2c a.sc-iol-header-b2c:hover,.dropdown-list.sc-iol-header-b2c button.sc-iol-header-b2c:hover{background-color:#FFFFFF}/*!@.dropdown-list button*/.dropdown-list.sc-iol-header-b2c button.sc-iol-header-b2c{position:relative}/*!@.dropdown-list button:after*/.dropdown-list.sc-iol-header-b2c button.sc-iol-header-b2c:after{content:\" \";display:block;width:24px;height:24px;mask-position:center center;-webkit-mask-position:center center;mask-image:url(\"data:image/svg+xml;base64,\");-webkit-mask-image:url(\"data:image/svg+xml;base64,\");mask-repeat:no-repeat;-webkit-mask-repeat:no-repeat;transform-box:fill-box;transform-origin:center;background-color:#232D3D;position:absolute;right:0;top:50%;margin-top:-12px}/*!@.menu*/.menu.sc-iol-header-b2c{align-items:center;flex-direction:row;display:none;margin-left:5rem}@media (min-width: 992px){/*!@.menu*/.menu.sc-iol-header-b2c{display:flex}}@media (min-width: 1200px){/*!@.menu*/.menu.sc-iol-header-b2c{margin-left:10rem}}/*!@.menu-separator*/.menu-separator.sc-iol-header-b2c{margin:0 1rem;border:0;border-right:1px solid #FFFFFF;height:1.5rem}/*!@.menu-link*/.menu-link.sc-iol-header-b2c{position:relative;color:#B2DBBF;line-height:3.75rem;padding-left:1rem;padding-right:1rem}/*!@.menu-link:hover*/.menu-link.sc-iol-header-b2c:hover{color:#F4E956}/*!@.menu-link:before*/.menu-link.sc-iol-header-b2c:before{content:\" \";position:absolute;width:100%;bottom:0;height:3px;left:0;background-color:#F4E956;opacity:0;transition:opacity 0.1s ease-in}/*!@.menu-link--dropdown*/.menu-link--dropdown.sc-iol-header-b2c{padding-right:calc(24px + 1.5rem)}/*!@.menu-link--dropdown:after*/.menu-link--dropdown.sc-iol-header-b2c:after{content:\" \";display:block;width:24px;height:24px;mask-position:center center;-webkit-mask-position:center center;mask-image:url(\"data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTIiIGhlaWdodD0iOCIgdmlld0JveD0iMCAwIDEyIDgiIGZpbGw9Im5vbmUiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+CjxwYXRoIGQ9Ik0xLjQxIDcuNDFMNiAyLjgzTDEwLjU5IDcuNDFMMTIgNkw2IDBMMCA2TDEuNDEgNy40MVoiIGZpbGw9IiMzMjMyMzIiLz4KPC9zdmc+Cg==\");-webkit-mask-image:url(\"data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTIiIGhlaWdodD0iOCIgdmlld0JveD0iMCAwIDEyIDgiIGZpbGw9Im5vbmUiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+CjxwYXRoIGQ9Ik0xLjQxIDcuNDFMNiAyLjgzTDEwLjU5IDcuNDFMMTIgNkw2IDBMMCA2TDEuNDEgNy40MVoiIGZpbGw9IiMzMjMyMzIiLz4KPC9zdmc+Cg==\");mask-repeat:no-repeat;-webkit-mask-repeat:no-repeat;transform-box:fill-box;transform-origin:center;background-color:#B2DBBF;position:absolute;right:1rem;top:50%;margin-top:-12px;transition:transform 0.1s ease-out;transform:rotate(180deg)}/*!@.dropdown-wrapper*/.dropdown-wrapper.sc-iol-header-b2c{align-items:center;height:3.75rem;position:relative;display:flex;flex-direction:column}/*!@.dropdown-wrapper:focus .dropdown-list,\n.dropdown-wrapper:focus-within .dropdown-list,\n.dropdown-wrapper:hover .dropdown-list*/.dropdown-wrapper.sc-iol-header-b2c:focus .dropdown-list.sc-iol-header-b2c,.dropdown-wrapper.sc-iol-header-b2c:focus-within .dropdown-list.sc-iol-header-b2c,.dropdown-wrapper.sc-iol-header-b2c:hover .dropdown-list.sc-iol-header-b2c{visibility:visible}/*!@.dropdown-wrapper:focus .menu-link,\n.dropdown-wrapper:focus-within .menu-link,\n.dropdown-wrapper--active .menu-link,\n.dropdown-wrapper:hover .menu-link,\n.menu-link:hover*/.dropdown-wrapper.sc-iol-header-b2c:focus .menu-link.sc-iol-header-b2c,.dropdown-wrapper.sc-iol-header-b2c:focus-within .menu-link.sc-iol-header-b2c,.dropdown-wrapper--active.sc-iol-header-b2c .menu-link.sc-iol-header-b2c,.dropdown-wrapper.sc-iol-header-b2c:hover .menu-link.sc-iol-header-b2c,.menu-link.sc-iol-header-b2c:hover{color:#F4E956}/*!@.dropdown-wrapper:focus .menu-link:before,\n.dropdown-wrapper:focus-within .menu-link:before,\n.dropdown-wrapper--active .menu-link:before,\n.dropdown-wrapper:hover .menu-link:before,\n.menu-link:hover:before*/.dropdown-wrapper.sc-iol-header-b2c:focus .menu-link.sc-iol-header-b2c:before,.dropdown-wrapper.sc-iol-header-b2c:focus-within .menu-link.sc-iol-header-b2c:before,.dropdown-wrapper--active.sc-iol-header-b2c .menu-link.sc-iol-header-b2c:before,.dropdown-wrapper.sc-iol-header-b2c:hover .menu-link.sc-iol-header-b2c:before,.menu-link.sc-iol-header-b2c:hover:before{opacity:1}/*!@.dropdown-wrapper:focus .menu-link:after,\n.dropdown-wrapper:focus-within .menu-link:after,\n.dropdown-wrapper--active .menu-link:after,\n.dropdown-wrapper:hover .menu-link:after,\n.menu-link:hover:after*/.dropdown-wrapper.sc-iol-header-b2c:focus .menu-link.sc-iol-header-b2c:after,.dropdown-wrapper.sc-iol-header-b2c:focus-within .menu-link.sc-iol-header-b2c:after,.dropdown-wrapper--active.sc-iol-header-b2c .menu-link.sc-iol-header-b2c:after,.dropdown-wrapper.sc-iol-header-b2c:hover .menu-link.sc-iol-header-b2c:after,.menu-link.sc-iol-header-b2c:hover:after{transform:rotate(0deg);background-color:#F4E956}/*!@.sub-menu-container*/.sub-menu-container.sc-iol-header-b2c{position:relative;display:flex}/*!@.sub-menu*/.sub-menu.sc-iol-header-b2c{padding:2rem;z-index:999;position:absolute;top:-2rem;left:calc(100% - 2rem + 0.5rem);visibility:hidden}/*!@.sub-menu-container:focus .sub-menu,\n.sub-menu-container:focus-within .sub-menu,\n.sub-menu-container:hover .sub-menu*/.sub-menu-container.sc-iol-header-b2c:focus .sub-menu.sc-iol-header-b2c,.sub-menu-container.sc-iol-header-b2c:focus-within .sub-menu.sc-iol-header-b2c,.sub-menu-container.sc-iol-header-b2c:hover .sub-menu.sc-iol-header-b2c{visibility:visible}/*!@.dropdown-list .sub-menu-button*/.dropdown-list.sc-iol-header-b2c .sub-menu-button.sc-iol-header-b2c{padding-right:calc(24px + 0.5rem);flex:1}/*!@.sub-menu__inner*/.sub-menu__inner.sc-iol-header-b2c{list-style:none;padding:0;margin:0;background:#F4E956;box-shadow:0 1px 8px 0 rgba(35, 45, 61, 0.3)}/*!@.mobile-menu-button-container, .account-button-container*/.mobile-menu-button-container.sc-iol-header-b2c,.account-button-container.sc-iol-header-b2c{display:flex;height:60px;align-items:center}@media (min-width: 992px){/*!@.mobile-menu-button-container, .account-button-container*/.mobile-menu-button-container.sc-iol-header-b2c,.account-button-container.sc-iol-header-b2c{display:none}}/*!@.mobile-menu-button*/.mobile-menu-button.sc-iol-header-b2c{position:relative;width:44px;height:44px;display:flex;justify-content:center;align-items:center}/*!@.mobile-menu-button__icon*/.mobile-menu-button__icon.sc-iol-header-b2c{fill:#F4E956;display:block;width:24px;height:24px;transform-box:fill-box;transform-origin:center}/*!@.mobile-menu__wrapper*/.mobile-menu__wrapper.sc-iol-header-b2c{position:fixed;left:0;top:0;width:100%;height:100%;visibility:hidden;z-index:2000}/*!@.mobile-menu__backdrop*/.mobile-menu__backdrop.sc-iol-header-b2c{position:absolute;left:0;top:0;width:100%;height:100%;background-color:rgba(18, 28, 44, 0.6);opacity:0;transition:visibility 0.3s, opacity 0.3s 0s;z-index:1}/*!@.mobile-menu__wrapper--visible*/.mobile-menu__wrapper--visible.sc-iol-header-b2c{visibility:visible}/*!@.mobile-menu__wrapper--visible .mobile-menu__backdrop*/.mobile-menu__wrapper--visible.sc-iol-header-b2c .mobile-menu__backdrop.sc-iol-header-b2c{opacity:1}/*!@.mobile-menu*/.mobile-menu.sc-iol-header-b2c{display:flex;flex-direction:column;background-color:#232D3D;position:absolute;top:0;right:0;width:100%;max-width:350px;height:100%;box-sizing:border-box;z-index:2;transform:translateX(100%);transition:visibility 0.3s, transform 0.3s ease-in-out;padding:1rem;overflow-y:auto}/*!@.mobile-menu__main a*/.mobile-menu__main.sc-iol-header-b2c a.sc-iol-header-b2c{color:#FFFFFF;text-transform:uppercase;line-height:55px;box-sizing:border-box;padding:0 1rem;display:inline-block}/*!@.mobile-menu__main*/.mobile-menu__main.sc-iol-header-b2c{margin-top:1rem}/*!@.mobile-menu__wrapper--visible .mobile-menu*/.mobile-menu__wrapper--visible.sc-iol-header-b2c .mobile-menu.sc-iol-header-b2c{transform:translateX(0)}/*!@.mobile-menu__header*/.mobile-menu__header.sc-iol-header-b2c{display:flex;flex-direction:row;align-items:center;justify-content:space-between}/*!@.mobile-menu__header img*/.mobile-menu__header.sc-iol-header-b2c img.sc-iol-header-b2c{width:118px;height:auto}/*!@.mobile-menu__close-button*/.mobile-menu__close-button.sc-iol-header-b2c{display:flex;justify-content:center;align-items:center}/*!@.mobile-menu__close-button-icon*/.mobile-menu__close-button-icon.sc-iol-header-b2c{fill:#F4E956;display:block;width:24px;height:24px;transform-box:fill-box;transform-origin:center}/*!@.mobile-menu__header-button*/.mobile-menu__header-button.sc-iol-header-b2c{width:100px;display:flex}/*!@.mobile-menu__header-button--close*/.mobile-menu__header-button--close.sc-iol-header-b2c{justify-content:flex-end}/*!@.mobile-menu__link-list*/.mobile-menu__link-list.sc-iol-header-b2c{list-style:none;padding:0;margin:0}/*!@.mobile-menu__link-list--account*/.mobile-menu__link-list--account.sc-iol-header-b2c{display:flex;flex-direction:column;margin-top:2rem;align-items:center}/*!@.mobile-menu .mobile-menu__link-list--account a*/.mobile-menu.sc-iol-header-b2c .mobile-menu__link-list--account.sc-iol-header-b2c a.sc-iol-header-b2c{color:#F4E956}/*!@.mobile-menu a.mobile-menu__link*/.mobile-menu.sc-iol-header-b2c a.mobile-menu__link.sc-iol-header-b2c{border-bottom:1px solid #FFFFFF;display:block;height:55px}/*!@.mobile-menu__employer-button*/.mobile-menu__employer-button.sc-iol-header-b2c{display:flex;align-items:flex-end;justify-content:center;flex:1;width:100%;padding:2rem 1rem 1rem;box-sizing:border-box}/*!@.mobile-menu__employer-button a*/.mobile-menu__employer-button.sc-iol-header-b2c a.sc-iol-header-b2c{background:#B2DBBF;transition:background-color 0.3s ease;display:flex;height:3rem;padding-left:1rem;padding-right:1rem;border-radius:3rem;min-width:12rem;justify-content:center;align-items:center;color:#232D3D;text-transform:uppercase}/*!@.mobile-menu__employer-button a:hover*/.mobile-menu__employer-button.sc-iol-header-b2c a.sc-iol-header-b2c:hover{text-decoration:none;background:#99C8A3}/*!@.collapsible-menu*/.collapsible-menu.sc-iol-header-b2c{border-bottom:1px solid #FFFFFF}/*!@.collapsible-menu__list*/.collapsible-menu__list.sc-iol-header-b2c{list-style:none;padding:0 0 0 1rem;margin:0}/*!@.collapsible-menu__toggle*/.collapsible-menu__toggle.sc-iol-header-b2c{display:block;cursor:pointer}/*!@.collapsible-menu .collapsible-menu__list-item*/.collapsible-menu.sc-iol-header-b2c .collapsible-menu__list-item.sc-iol-header-b2c{display:block}/*!@.collapsible-menu__toggle*/.collapsible-menu__toggle.sc-iol-header-b2c{position:relative}/*!@.collapsible-menu__toggle:after*/.collapsible-menu__toggle.sc-iol-header-b2c:after{content:\" \";display:block;width:24px;height:24px;mask-position:center center;-webkit-mask-position:center center;mask-image:url(\"data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTIiIGhlaWdodD0iOCIgdmlld0JveD0iMCAwIDEyIDgiIGZpbGw9Im5vbmUiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+CjxwYXRoIGQ9Ik0xLjQxIDcuNDFMNiAyLjgzTDEwLjU5IDcuNDFMMTIgNkw2IDBMMCA2TDEuNDEgNy40MVoiIGZpbGw9IiMzMjMyMzIiLz4KPC9zdmc+Cg==\");-webkit-mask-image:url(\"data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTIiIGhlaWdodD0iOCIgdmlld0JveD0iMCAwIDEyIDgiIGZpbGw9Im5vbmUiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+CjxwYXRoIGQ9Ik0xLjQxIDcuNDFMNiAyLjgzTDEwLjU5IDcuNDFMMTIgNkw2IDBMMCA2TDEuNDEgNy40MVoiIGZpbGw9IiMzMjMyMzIiLz4KPC9zdmc+Cg==\");mask-repeat:no-repeat;-webkit-mask-repeat:no-repeat;transform-box:fill-box;transform-origin:center;background-color:#FFFFFF;position:absolute;right:0;top:50%;margin-top:calc(24px * -0.5);transition:transform 0.1s ease-out;transform:rotate(180deg)}/*!@.collapsible-menu__toggle--open:after*/.collapsible-menu__toggle--open.sc-iol-header-b2c:after{transform:rotate(0deg)}";

class IolHeaderB2c {
  constructor(hostRef) {
    registerInstance(this, hostRef);
    this.isActiveMobileMenu = false;
    this.isOpenMobileJobsMenu = false;
    this.isOpenMobileTopicsMenu = false;
    this.isOpenMobileEducationMenu = false;
  }
  ...
  handleToggleCollapsible(collapsibleId) {
  ...
  render() {
    ...
    return (hAsync("header", null, hAsync("div", { class: "container container--top" }, hAsync(TopBar, { emailAddress: this.emailAddress })), hAsync("nav", { class: 'main' }, hAsync("div", { class: "container" }, hAsync("div", { class: "main-inner" }, hAsync(Logo$1, null), hAsync("div", { class: "mobile-menu-button-container" }, hAsync("button", { class: "mobile-menu-button", onClick: () => this.handleOpenMenu(), "aria-label": "Open menu" }, hAsync(Icons$1.menu, { className: 'mobile-menu-button__icon' }))), hAsync("div", { class: 'menu' }, hAsync("div", { class: dropdownActiveClassExtender(topicsListItems.map(item => item.value)) }, hAsync(MainMenuLink, { item: mainMenuLinks.TOPICS, withDropdown: true }), hAsync(DropdownContainer$1, { listItems: topicsListItems, analyticsAction: 'ClickOnHeaderTopicsLink' })), hAsync(MainMenuLink, { item: mainMenuLinks.TESTEN_EN_TOOLS }), hAsync("div", { class: dropdownActiveClassExtender(topicsListItems.map(item => item.value)) }, hAsync(MainMenuLink, { item: mainMenuLinks.OPLEIDINGEN, withDropdown: true }), hAsync(DropdownContainer$1, { listItems: educationItems, analyticsAction: 'ClickOnHeaderOpleidingenLink' })), hAsync("div", { class: dropdownActiveClassExtender('/vacature') }, hAsync(MainMenuLink, { item: mainMenuLinks.VACATURES, withDropdown: true }), hAsync(DropdownContainer, null)), hAsync(MainMenuLink, { item: mainMenuLinks.JOUW_PROFIEL }))))), hAsync(MobileMenu$1, { isAuthenticated: !!this.emailAddress, isVisible: this.isActiveMobileMenu, handleModalClose: () => this.handleClose(), isOpenJobsMenu: this.isOpenMobileJobsMenu, isOpenTopicsMenu: this.isOpenMobileTopicsMenu, isOpenEducationMenu: this.isOpenMobileEducationMenu, handleToggleCollapsible: id => this.handleToggleCollapsible(id) })));
  }
  static get watchers() { return {
    "isActiveMobileMenu": ["adjustDocument"]
  }; }
  static get style() { return iolHeaderB2cCss; }
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "iol-header-b2c",
    "$members$": {
      "emailAddress": [1, "email-address"],
      "isActiveMobileMenu": [32],
      "isOpenMobileJobsMenu": [32],
      "isOpenMobileTopicsMenu": [32],
      "isOpenMobileEducationMenu": [32]
    },
    "$listeners$": undefined,
    "$lazyBundleId$": "-",
    "$attrsToReflect$": []
...
const myComponentCss = "/*!@div*/div.sc-my-component{display:block}/*!@button*/button.sc-my-component{color:#00799b}";
...
const Logo = () => hAsync("a", Object.assign({ href: "/", class: 'logo-link' }, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderLink', label: 'Home' })),
...
const SubMenu = (analyticsAction) => ({ listItems, subMenuTitle, isCapitalized }) => hAsync("div", { class: 'sub-menu' },
  hAsync("div", { class: 'sub-menu__inner' },
    hAsync("strong", { class: 'sub-menu__title' }, subMenuTitle),
    hAsync("ul", { class: 'sub-menu__list' },
      hAsync(AnchorList, { listItems: listItems, analyticsAction: analyticsAction, className: isCapitalized ? 'sub-menu__list-item--capitalized' : '' }))));
  ...
  hAsync("div", { class: "container" },
    hAsync("ul", { class: 'dropdown-list' },
      ...
      hAsync("li", { class: 'sub-menu-container' },
        ...
        hAsync(JobSeekerSubMenu, { listItems: cityListItems, subMenuTitle: 'Per plaats', isCapitalized: true })),
...
const DropdownContainerAccountAuthenticated$1 = () => hAsync("div", { class: "dropdown-container" },
  hAsync("div", { class: "container" },
    hAsync("ul", { class: 'dropdown-list' },
      hAsync("li", null,
        hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: "Zoekopdrachten" }), { href: "/vacature/zoeken/zoekopdrachten-beheren" }), "Zoekopdrachten")),
      hAsync("li", null,
        hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: "Favorieten" }), { href: "/vacature/zoeken/favorieten" }), "Favorieten")),
      hAsync("li", null,
        hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: "JouwProfiel" }), { href: "/profiel/overzicht" }), "Jouw profiel")),
      hAsync("li", null,
        hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: "JouwEmailInstellingen" }), { href: "/account/e-mailinstellingen" }), "E-mailinstellingen")),
      hAsync("li", null,
        hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: "JouwAccount" }), { href: "/account" }), "Jouw account")),
      hAsync("li", null,
        hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: "Uitloggen" }), { href: "/account/uitloggen" }), "Uitloggen")))));
...
const MobileSubMenu = ({ listItems, subMenuTitle, capitalizeLabels }) => hAsync("div", null,
  ...
  hAsync("ul", { class: 'mobile-menu__link-list' + (capitalizeLabels ? ' mobile-menu__link-list--capitalize' : '') },
    hAsync(AnchorList, { className: "mobile-menu__link--no-icon", listItems: listItems, analyticsAction: 'ClickOnHeaderWerkZoekendenLink' })));
...
const MobileJobsMenu = ({ setActiveMobileMenuLevel }) => hAsync("div", null,
  ...
  hAsync("ul", { class: 'mobile-menu__link-list' },
    ...
    hAsync("li", null,
      hAsync("button", { class: "mobile-menu__link--no-icon", onClick: () => setActiveMobileMenuLevel('industries') }, "Per branche"))));

const paths = {
  'campaign': () => hAsync("path", { d: "M18 11v2h4v-2h-4ZM16 17.61c.96.71 2.21 1.65 3.2 2.39.4-.53.8-1.07 1.2-1.6-.99-.74-2.24-1.68-3.2-2.4-.4.54-.8 1.08-1.2 1.61ZM20.4 5.6c-.4-.53-.8-1.07-1.2-1.6-.99.74-2.24 1.68-3.2 2.4.4.53.8 1.07 1.2 1.6.96-.72 2.21-1.65 3.2-2.4ZM4 9c-1.1 0-2 .9-2 2v2c0 1.1.9 2 2 2h1v4h2v-4h1l5 3V6L8 9H4Zm5.03 1.71L11 9.53v4.94l-1.97-1.18-.48-.29H4v-2h4.55l.48-.29ZM15.5 12c0-1.33-.58-2.53-1.5-3.35v6.69c.92-.81 1.5-2.01 1.5-3.34Z" }),
  'search': () => hAsync("path", { d: "M15.5 14h-.79l-.28-.27a6.5 6.5 0 0 0 1.48-5.34c-.47-2.78-2.79-5-5.59-5.34a6.505 6.505 0 0 0-7.27 7.27c.34 2.8 2.56 5.12 5.34 5.59a6.5 6.5 0 0 0 5.34-1.48l.27.28v.79l4.25 4.25c.41.41 1.08.41 1.49 0 .41-.41.41-1.08 0-1.49L15.5 14Zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14Z" }),
  'euro': () => hAsync("path", { d: "M15 18.5A6.48 6.48 0 0 1 9.24 15H15l1-2H8.58c-.05-.33-.08-.66-.08-1 0-.34.03-.67.08-1H15l1-2H9.24A6.491 6.491 0 0 1 15 5.5c1.61 0 3.09.59 4.23 1.57L21 5.3A8.955 8.955 0 0 0 15 3c-3.92 0-7.24 2.51-8.48 6H3l-1 2h4.06a8.262 8.262 0 0 0 0 2H3l-1 2h4.52c1.24 3.49 4.56 6 8.48 6 2.31 0 4.41-.87 6-2.3l-1.78-1.77c-1.13.98-2.6 1.57-4.22 1.57Z" }),
  'lightbulb': () => hAsync("path", { d: "M9 21c0 .55.45 1 1 1h4c.55 0 1-.45 1-1v-1H9v1Zm3-19C8.14 2 5 5.14 5 9c0 2.38 1.19 4.47 3 5.74V17c0 .55.45 1 1 1h6c.55 0 1-.45 1-1v-2.26c1.81-1.27 3-3.36 3-5.74 0-3.86-3.14-7-7-7Zm2.85 11.1-.85.6V16h-4v-2.3l-.85-.6A4.997 4.997 0 0 1 7 9c0-2.76 2.24-5 5-5s5 2.24 5 5c0 1.63-.8 3.16-2.15 4.1Z" }),
  'settings': () => hAsync("path", { d: "M19.43 12.98c.04-.32.07-.64.07-.98 0-.34-.03-.66-.07-.98l2.11-1.65c.19-.15.24-.42.12-.64l-2-3.46a.5.5 0 0 0-.61-.22l-2.49 1c-.52-.4-1.08-.73-1.69-.98l-.38-2.65A.488.488 0 0 0 14 2h-4c-.25 0-.46.18-.49.42l-.38 2.65c-.61.25-1.17.59-1.69.98l-2.49-1a.566.566 0 0 0-.18-.03c-.17 0-.34.09-.43.25l-2 3.46c-.13.22-.07.49.12.64l2.11 1.65c-.04.32-.07.65-.07.98 0 .33.03.66.07.98l-2.11 1.65c-.19.15-.24.42-.12.64l2 3.46a.5.5 0 0 0 .61.22l2.49-1c.52.4 1.08.73 1.69.98l.38 2.65c.03.24.24.42.49.42h4c.25 0 .46-.18.49-.42l.38-2.65c.61-.25 1.17-.59 1.69-.98l2.49 1c.06.02.12.03.18.03.17 0 .34-.09.43-.25l2-3.46c.12-.22.07-.49-.12-.64l-2.11-1.65Zm-1.98-1.71c.04.31.05.52.05.73 0 .21-.02.43-.05.73l-.14 1.13.89.7 1.08.84-.7 1.21-1.27-.51-1.04-.42-.9.68c-.43.32-.84.56-1.25.73l-1.06.43-.16 1.13-.2 1.35h-1.4l-.19-1.35-.16-1.13-1.06-.43c-.43-.18-.83-.41-1.23-.71l-.91-.7-1.06.43-1.27.51-.7-1.21 1.08-.84.89-.7-.14-1.13c-.03-.31-.05-.54-.05-.74 0-.2.02-.43.05-.73l.14-1.13-.89-.7-1.08-.84.7-1.21 1.27.51 1.04.42.9-.68c.43-.32.84-.56 1.25-.73l1.06-.43.16-1.13.2-1.35h1.39l.19 1.35.16 1.13 1.06.43c.43.18.83.41 1.23.71l.91.7 1.06-.43 1.27-.51.7 1.21-1.07.85-.89.7.14 1.13ZM12 8c-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4-1.79-4-4-4Zm0 6c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2Z" }),
  'mail': () => hAsync("path", { d: "M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2Zm-1 14H5c-.55 0-1-.45-1-1V8l6.94 4.34c.65.41 1.47.41 2.12 0L20 8v9c0 .55-.45 1-1 1Zm-7-7L4 6h16l-8 5Z" }),
  'favorite': () => hAsync("path", { d: "M16.5 3c-1.74 0-3.41.81-4.5 2.09C10.91 3.81 9.24 3 7.5 3 4.42 3 2 5.42 2 8.5c0 3.78 3.4 6.86 8.55 11.54L12 21.35l1.45-1.32C18.6 15.36 22 12.28 22 8.5 22 5.42 19.58 3 16.5 3Zm-4.4 15.55-.1.1-.1-.1C7.14 14.24 4 11.39 4 8.5 4 6.5 5.5 5 7.5 5c1.54 0 3.04.99 3.57 2.36h1.87C13.46 5.99 14.96 5 16.5 5c2 0 3.5 1.5 3.5 3.5 0 2.89-3.14 5.74-7.9 10.05Z" }),
  'person': () => hAsync("path", { d: "M12 5.9a2.1 2.1 0 1 1 0 4.2 2.1 2.1 0 0 1 0-4.2Zm0 9c2.97 0 6.1 1.46 6.1 2.1v1.1H5.9V17c0-.64 3.13-2.1 6.1-2.1ZM12 4C9.79 4 8 5.79 8 8s1.79 4 4 4 4-1.79 4-4-1.79-4-4-4Zm0 9c-2.67 0-8 1.34-8 4v2c0 .55.45 1 1 1h14c.55 0 1-.45 1-1v-2c0-2.66-5.33-4-8-4Z" }),
  'menu': () => hAsync("path", { d: "M0 4h24v2H0zM0 11h24v2H0zM0 18h24v2H0z" }),
  'pencil': () => hAsync("path", { d: "M0 15.25V19h3.75L14.81 7.94l-3.75-3.75L0 15.25ZM18.41 4.34 14.66.59l-2.53 2.54 3.75 3.75 2.53-2.54Z" }),
};
const Icon = (path) => ({ className }) => hAsync("svg", { class: className, width: "24", height: "24", fill: "#323232", xmlns: "http://www.w3.org/2000/svg" }, path());
const Icons = {
...
const MobileMenuAccountAuthenticated$1 = () => hAsync("ul", { class: 'mobile-menu__link-list' },
  hAsync("li", null,
    hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: 'JouwProfiel' }), { class: 'mobile-menu__link--with-icon', href: '/profiel/overzicht' }),
      hAsync(Icons.person, { className: 'mobile-menu__link-icon' }),
      "Profiel")),
  hAsync("li", null,
    hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: 'Favorieten' }), { class: 'mobile-menu__link--with-icon', href: '/vacature/zoeken/favorieten' }),
      hAsync(Icons.favorite, { className: 'mobile-menu__link-icon' }),
      "Favorieten")),
  hAsync("li", null,
    hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: 'Zoekopdrachten' }), { class: 'mobile-menu__link--with-icon', href: '/vacature/zoeken/zoekopdrachten-beheren' }),
      hAsync(Icons.mail, { className: 'mobile-menu__link-icon' }),
      "Zoekopdrachten")),
  hAsync("li", null,
    hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: 'JouwEmailInstellingen' }), { class: 'mobile-menu__link--with-icon', href: '/account/e-mailinstellingen' }),
      hAsync(Icons.settings, { className: 'mobile-menu__link-icon' }),
      "E-mailinstellingen")),
  hAsync("li", null,
    hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: 'Uitloggen' }), { class: 'mobile-menu__link--no-icon', href: '/account/uitloggen' }), "Uitloggen")));

const MobileMenuAccountAuthenticated = () => hAsync("ul", { class: 'mobile-menu__link-list' },
  hAsync("li", null,
    hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: 'Inloggen' }), { class: 'mobile-menu__link--with-icon', href: '/account/inloggen' }),
      hAsync(Icons.person, { className: 'mobile-menu__link-icon' }),
      "Inloggen")),
  hAsync("li", null,
    hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: 'Registreren' }), { class: 'mobile-menu__link--with-icon', href: '/profiel/registreren' }),
      hAsync(Icons.pencil, { className: 'mobile-menu__link-icon' }),
      "Registreren")));

const MobileMainMenu = ({ setActiveMobileMenuLevel, isAuthenticated }) => hAsync("div", null,
  hAsync("div", { class: 'mobile-menu__link-list-title' }, "Carri\u00E8re"),
  hAsync("ul", { class: 'mobile-menu__link-list' },
    hAsync("li", null,
      hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderWerkZoekendenLink', label: 'Vacatures' }), { class: 'mobile-menu__link--with-icon', href: "/vacature/zoeken" }),
    ...
    hAsync("li", null,
      hAsync("button", { class: 'mobile-menu__link--with-icon', onClick: () => setActiveMobileMenuLevel('career') },
        ...
        "Carri\u00E8retips")),
    hAsync("li", null,
      hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderWerkZoekendenLink', label: 'Salariswijzer' }), { class: 'mobile-menu__link--with-icon', href: "/salariswijzer" }),
        hAsync(Icons.euro, { className: 'mobile-menu__link-icon' }),
        "Salariswijzer"))),
  hAsync("div", { class: 'mobile-menu__link-list-title' }, "Account"),
  isAuthenticated && hAsync(MobileMenuAccountAuthenticated$1, null),
  !isAuthenticated && hAsync(MobileMenuAccountAuthenticated, null),
  hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderWerkgeversLink', label: 'VoorWerkgevers' }), { class: 'mobile-menu__bottom-link', href: '/werkgever' }), "Werkgevers / plaats vacature"));

const secondLevelIds = ['jobs', 'career'];
const thirdLevelIds = ['cities', 'industries'];
const MobileMenu = ({ isVisible, handleModalClose, activeMobileMenuLevel, setActiveMobileMenuLevel, isAuthenticated }) => {
  let stackInnerModifier = '';
  let previousLevel;
  if (secondLevelIds.indexOf(activeMobileMenuLevel) > -1) {
    stackInnerModifier = ' mobile-menu__stack-inner--screen2';
    previousLevel = 'main';
  }
  else if (thirdLevelIds.indexOf(activeMobileMenuLevel) > -1) {
  ...
  return hAsync("div", { class: isVisible ? 'mobile-menu__wrapper mobile-menu__wrapper--visible' : 'mobile-menu__wrapper' },
    ...
    hAsync("nav", { class: "mobile-menu" },
      hAsync("div", { class: "mobile-menu__header" },
        ...
        hAsync("div", { class: 'mobile-menu__header-center-placeholder ' }),
        hAsync("div", { class: "mobile-menu__header-button mobile-menu__header-button--close" },
          hAsync("button", { onClick: () => handleModalClose(), class: "mobile-menu__close-button" }))),
      hAsync("div", { class: "mobile-menu__stack" },
        hAsync("div", { class: "mobile-menu__stack-inner" + stackInnerModifier },
          hAsync("section", { class: "mobile-menu__stack-screen" },
            hAsync(MobileMainMenu, { setActiveMobileMenuLevel: setActiveMobileMenuLevel, isAuthenticated: isAuthenticated })),
          hAsync("section", { class: "mobile-menu__stack-screen" },
            activeMobileMenuLevel === 'jobs' && hAsync(MobileJobsMenu, { setActiveMobileMenuLevel: setActiveMobileMenuLevel }),
            activeMobileMenuLevel === 'career' && hAsync(MobileCareerMenu, null)),
          hAsync("section", { class: "mobile-menu__stack-screen" },
            activeMobileMenuLevel === 'cities' && hAsync(MobileCitiesMenu, null),
            activeMobileMenuLevel === 'industries' && hAsync(MobileIndustriesMenu, null))))));
};

const DropdownContainerAccountAuthenticated = () => hAsync("div", { class: "dropdown-container" },
  hAsync("div", { class: "container" },
    hAsync("ul", { class: 'dropdown-list' },
      hAsync("li", null,
        hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: "Inloggen" }), { href: "/account/inloggen" }), "Inloggen")),
      hAsync("li", null,
        hAsync("a", Object.assign({}, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderAccountLink', label: "Registreren" }), { href: "/profiel/registreren" }), "Registreren")))));

const AccountButton = () => hAsync("a", Object.assign({ class: "account-button", href: "/account", "aria-label": "Naar account" }, AnalyticsDataAttributes$1({
  action: 'ClickOnHeaderLink',
  label: 'Account'
})),
  hAsync(Icons.person, { className: 'account-button__icon' }));

const nvbHeaderB2cCss = "/*!@header*/header.sc-nvb-header-b2c{all:initial;font-family:Manrope, sans-serif;display:flex;flex-direction:column;align-items:stretch}/*!@a*/a.sc-nvb-header-b2c{text-decoration:none}/*!@a:hover*/a.sc-nvb-header-b2c:hover{text-decoration-line:underline}@supports (contain: none){/*!@a:focus-visible*/a.sc-nvb-header-b2c:focus-visible{outline:none;outline:1px solid #000}}@supports not (contain: none){/*!@a:focus*/a.sc-nvb-header-b2c:focus{outline:none;outline:1px solid #000}}/*!@button*/button.sc-nvb-header-b2c{font-family:inherit;font-size:100%;line-height:1.15;margin:0;overflow:visible;text-transform:none;-webkit-appearance:button;-webkit-appearance:none;background:none;border:0;cursor:pointer;padding:0;text-decoration:none;box-sizing:content-box}/*!@button:hover*/button.sc-nvb-header-b2c:hover{text-decoration-line:underline}@supports (contain: none){/*!@button:focus-visible*/button.sc-nvb-header-b2c:focus-visible{outline:none;outline:1px solid #000}}@supports not (contain: none){/*!@button:focus*/button.sc-nvb-header-b2c:focus{outline:none;outline:1px solid #000}}/*!@.container*/.container.sc-nvb-header-b2c{box-sizing:border-box;max-width:75rem;width:100%;margin-left:auto;margin-right:auto;padding-left:0.5rem;padding-right:0.5rem}@media (min-width: 992px){/*!@.container*/.container.sc-nvb-header-b2c{padding-left:1.375rem;padding-right:1.375rem}}/*!@.container--top*/.container--top.sc-nvb-header-b2c{display:none}@media (min-width: 992px){/*!@.container--top*/.container--top.sc-nvb-header-b2c{display:block}}/*!@.top*/.top.sc-nvb-header-b2c{display:flex;justify-content:flex-end;align-items:center;height:2rem;padding-right:0.6rem}/*!@.top a*/.top.sc-nvb-header-b2c a.sc-nvb-header-b2c{color:#858585;text-transform:lowercase;font-size:0.875rem}/*!@.main*/.main.sc-nvb-header-b2c{background-color:#05baed}/*!@.main-inner*/.main-inner.sc-nvb-header-b2c{display:flex;flex-direction:row;justify-content:space-between;align-items:stretch}/*!@.logo-link*/.logo-link.sc-nvb-header-b2c{display:flex;align-items:center}/*!@.mobile-menu-button-container, .account-button-container*/.mobile-menu-button-container.sc-nvb-header-b2c,.account-button-container.sc-nvb-header-b2c{display:flex;height:60px;align-items:center}@media (min-width: 992px){/*!@.mobile-menu-button-container, .account-button-container*/.mobile-menu-button-container.sc-nvb-header-b2c,.account-button-container.sc-nvb-header-b2c{display:none}}/*!@.mobile-menu-button, .account-button*/.mobile-menu-button.sc-nvb-header-b2c,.account-button.sc-nvb-header-b2c{position:relative;width:44px;height:44px;display:flex;justify-content:center;align-items:center}/*!@.mobile-menu-button__icon, .account-button__icon*/.mobile-menu-button__icon.sc-nvb-header-b2c,.account-button__icon.sc-nvb-header-b2c{fill:#fff;display:block;width:24px;height:24px;transform-box:fill-box;transform-origin:center}/*!@.dropdown-container*/.dropdown-container.sc-nvb-header-b2c{position:absolute;width:100%;top:5.75rem;left:0;visibility:hidden;background:#f6f6f6;z-index:1000}/*!@.dropdown-container a,\n.dropdown-container button*/.dropdown-container.sc-nvb-header-b2c a.sc-nvb-header-b2c,.dropdown-container.sc-nvb-header-b2c button.sc-nvb-header-b2c{display:block;line-height:2.75rem;text-align:left;text-transform:lowercase;color:#09181b;padding-left:0.5rem;margin-left:-0.5rem;width:13rem;box-sizing:border-box}/*!@.dropdown-container a:hover,\n.dropdown-container button:hover*/.dropdown-container.sc-nvb-header-b2c a.sc-nvb-header-b2c:hover,.dropdown-container.sc-nvb-header-b2c button.sc-nvb-header-b2c:hover{background-color:#b9efff;text-decoration:none}/*!@.dropdown-container button*/.dropdown-container.sc-nvb-header-b2c button.sc-nvb-header-b2c{position:relative}/*!@.dropdown-container button:after*/.dropdown-container.sc-nvb-header-b2c button.sc-nvb-header-b2c:after{content:\" \";display:block;width:24px;height:24px;mask-position:center center;-webkit-mask-position:center center;mask-image:url(\"data:image/svg+xml;base64,\");-webkit-mask-image:url(\"data:image/svg+xml;base64,\");mask-repeat:no-repeat;-webkit-mask-repeat:no-repeat;transform-box:fill-box;transform-origin:center;background-color:#000;position:absolute;right:0;top:50%;margin-top:-12px}/*!@.dropdown-backdrop*/.dropdown-backdrop.sc-nvb-header-b2c{z-index:999;display:block;visibility:hidden;opacity:0;position:absolute;background:rgba(0, 0, 0, 0.6);top:5.75rem;left:0;width:100%;height:calc(100% - 3.75rem - 2rem);transition:visibility 0.3s, opacity 0.3s 0s}/*!@.menu*/.menu.sc-nvb-header-b2c{align-items:center;flex-direction:row;width:35rem;display:none}@media (min-width: 992px){/*!@.menu*/.menu.sc-nvb-header-b2c{display:flex}}/*!@.menu-separator*/.menu-separator.sc-nvb-header-b2c{margin:0 1rem;border:0;border-right:1px solid #fff;height:1.5rem}/*!@.menu-link*/.menu-link.sc-nvb-header-b2c{position:relative;color:#fff;line-height:3.75rem;text-transform:lowercase;margin-left:1rem;margin-right:1rem}/*!@.menu-link:hover*/.menu-link.sc-nvb-header-b2c:hover{text-decoration:none}/*!@.menu-link:before*/.menu-link.sc-nvb-header-b2c:before{content:\" \";position:absolute;width:100%;bottom:6px;height:3px;left:0;background-color:#fff;border-radius:4px;opacity:0;transition:opacity 0.1s ease-in}/*!@.menu-link--dropdown*/.menu-link--dropdown.sc-nvb-header-b2c{padding-right:calc(24px + 0.5rem)}/*!@.menu-link--dropdown:after*/.menu-link--dropdown.sc-nvb-header-b2c:after{content:\" \";display:block;width:24px;height:24px;mask-position:center center;-webkit-mask-position:center center;mask-image:url(\"data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTIiIGhlaWdodD0iOCIgdmlld0JveD0iMCAwIDEyIDgiIGZpbGw9Im5vbmUiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+CjxwYXRoIGQ9Ik0xLjQxIDcuNDFMNiAyLjgzTDEwLjU5IDcuNDFMMTIgNkw2IDBMMCA2TDEuNDEgNy40MVoiIGZpbGw9IiMzMjMyMzIiLz4KPC9zdmc+Cg==\");-webkit-mask-image:url(\"data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTIiIGhlaWdodD0iOCIgdmlld0JveD0iMCAwIDEyIDgiIGZpbGw9Im5vbmUiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+CjxwYXRoIGQ9Ik0xLjQxIDcuNDFMNiAyLjgzTDEwLjU5IDcuNDFMMTIgNkw2IDBMMCA2TDEuNDEgNy40MVoiIGZpbGw9IiMzMjMyMzIiLz4KPC9zdmc+Cg==\");mask-repeat:no-repeat;-webkit-mask-repeat:no-repeat;transform-box:fill-box;transform-origin:center;background-color:#fff;position:absolute;right:0;top:50%;margin-top:-12px;transition:transform 0.1s ease-out;transform:rotate(180deg)}/*!@.dropdown-wrapper*/.dropdown-wrapper.sc-nvb-header-b2c{display:flex;align-items:center;height:3.75rem}/*!@.dropdown-wrapper:focus .dropdown-container,\n.dropdown-wrapper:focus-within .dropdown-container,\n.dropdown-wrapper:hover .dropdown-container*/.dropdown-wrapper.sc-nvb-header-b2c:focus .dropdown-container.sc-nvb-header-b2c,.dropdown-wrapper.sc-nvb-header-b2c:focus-within .dropdown-container.sc-nvb-header-b2c,.dropdown-wrapper.sc-nvb-header-b2c:hover .dropdown-container.sc-nvb-header-b2c{visibility:visible}/*!@.dropdown-wrapper:focus ~ .dropdown-backdrop,\n.dropdown-wrapper:focus-within ~ .dropdown-backdrop,\n.dropdown-wrapper:hover ~ .dropdown-backdrop*/.dropdown-wrapper.sc-nvb-header-b2c:focus~.dropdown-backdrop.sc-nvb-header-b2c,.dropdown-wrapper.sc-nvb-header-b2c:focus-within~.dropdown-backdrop.sc-nvb-header-b2c,.dropdown-wrapper.sc-nvb-header-b2c:hover~.dropdown-backdrop.sc-nvb-header-b2c{visibility:visible;opacity:1}/*!@.dropdown-wrapper:focus .menu-link:after,\n.dropdown-wrapper:focus-within .menu-link:after,\n.dropdown-wrapper:hover .menu-link:after*/.dropdown-wrapper.sc-nvb-header-b2c:focus .menu-link.sc-nvb-header-b2c:after,.dropdown-wrapper.sc-nvb-header-b2c:focus-within .menu-link.sc-nvb-header-b2c:after,.dropdown-wrapper.sc-nvb-header-b2c:hover .menu-link.sc-nvb-header-b2c:after{transform:rotate(0deg)}/*!@.dropdown-wrapper:focus .menu-link:before,\n.dropdown-wrapper:focus-within .menu-link:before,\n.dropdown-wrapper--active .menu-link:before,\n.dropdown-wrapper:hover .menu-link:before,\n.menu-link:hover:before*/.dropdown-wrapper.sc-nvb-header-b2c:focus .menu-link.sc-nvb-header-b2c:before,.dropdown-wrapper.sc-nvb-header-b2c:focus-within .menu-link.sc-nvb-header-b2c:before,.dropdown-wrapper--active.sc-nvb-header-b2c .menu-link.sc-nvb-header-b2c:before,.dropdown-wrapper.sc-nvb-header-b2c:hover .menu-link.sc-nvb-header-b2c:before,.menu-link.sc-nvb-header-b2c:hover:before{opacity:1}/*!@.dropdown-list*/.dropdown-list.sc-nvb-header-b2c{padding:2.75rem 0;margin-left:calc(100% - 35rem + 1.375rem - 0.5rem);position:relative;list-style:none}/*!@.sub-menu-container:focus .sub-menu,\n.sub-menu-container:focus-within .sub-menu,\n.sub-menu-container:hover .sub-menu*/.sub-menu-container.sc-nvb-header-b2c:focus .sub-menu.sc-nvb-header-b2c,.sub-menu-container.sc-nvb-header-b2c:focus-within .sub-menu.sc-nvb-header-b2c,.sub-menu-container.sc-nvb-header-b2c:hover .sub-menu.sc-nvb-header-b2c{visibility:visible}/*!@.dropdown-list-expander*/.dropdown-list-expander.sc-nvb-header-b2c{display:none}/*!@.dropdown-list-expander--jobs*/.dropdown-list-expander--jobs.sc-nvb-header-b2c{height:13.75rem}/*!@.sub-menu-container:focus ~ .dropdown-list-expander,\n.sub-menu-container:focus-within ~ .dropdown-list-expander,\n.sub-menu-container:hover ~ .dropdown-list-expander*/.sub-menu-container.sc-nvb-header-b2c:focus~.dropdown-list-expander.sc-nvb-header-b2c,.sub-menu-container.sc-nvb-header-b2c:focus-within~.dropdown-list-expander.sc-nvb-header-b2c,.sub-menu-container.sc-nvb-header-b2c:hover~.dropdown-list-expander.sc-nvb-header-b2c{display:block}/*!@.sub-menu-container:hover button*/.sub-menu-container.sc-nvb-header-b2c:hover button.sc-nvb-header-b2c{background-color:#b9efff;text-decoration:none}/*!@.sub-menu*/.sub-menu.sc-nvb-header-b2c{display:flex;position:absolute;min-height:100%;left:13.5rem;top:0;min-width:300px;padding-left:4rem;padding-top:2.75rem;padding-bottom:2.75rem;visibility:hidden;box-sizing:border-box}/*!@.sub-menu__inner*/.sub-menu__inner.sc-nvb-header-b2c{position:relative}/*!@.sub-menu__inner:before*/.sub-menu__inner.sc-nvb-header-b2c:before{content:\" \";display:block;position:absolute;left:-2rem;border-left:#c9c9c9 1px solid;height:100%;top:0}/*!@.sub-menu__title*/.sub-menu__title.sc-nvb-header-b2c{line-height:2.75rem;text-transform:lowercase}/*!@.sub-menu__list*/.sub-menu__list.sc-nvb-header-b2c{list-style:none;padding:0;margin:0}/*!@.sub-menu .sub-menu__list-item--capitalized*/.sub-menu.sc-nvb-header-b2c .sub-menu__list-item--capitalized.sc-nvb-header-b2c{text-transform:capitalize}/*!@.mobile-menu__wrapper*/.mobile-menu__wrapper.sc-nvb-header-b2c{position:fixed;left:0;top:0;width:100%;height:100%;visibility:hidden;z-index:2000}/*!@.mobile-menu__backdrop*/.mobile-menu__backdrop.sc-nvb-header-b2c{position:absolute;left:0;top:0;width:100%;height:100%;background-color:rgba(0, 0, 0, 0.6);opacity:0;transition:visibility 0.3s, opacity 0.3s 0s;z-index:1}/*!@.mobile-menu__wrapper--visible*/.mobile-menu__wrapper--visible.sc-nvb-header-b2c{visibility:visible}/*!@.mobile-menu__wrapper--visible .mobile-menu__backdrop*/.mobile-menu__wrapper--visible.sc-nvb-header-b2c .mobile-menu__backdrop.sc-nvb-header-b2c{opacity:1}/*!@.mobile-menu*/.mobile-menu.sc-nvb-header-b2c{display:flex;flex-direction:column;background-color:#fff;position:absolute;top:0;left:0;width:100%;max-width:350px;height:100%;box-sizing:border-box;z-index:2;transform:translateX(-100%);transition:visibility 0.3s, transform 0.3s ease-in-out}/*!@.mobile-menu__wrapper--visible .mobile-menu*/.mobile-menu__wrapper--visible.sc-nvb-header-b2c .mobile-menu.sc-nvb-header-b2c{transform:translateX(0)}/*!@.mobile-menu__header*/.mobile-menu__header.sc-nvb-header-b2c{display:flex;flex-direction:row;align-items:center;height:60px;padding-left:0.5rem;padding-right:0.5rem;border-bottom:1px solid #c9c9c9}/*!@.mobile-menu__header img*/.mobile-menu__header.sc-nvb-header-b2c img.sc-nvb-header-b2c{width:118px;height:auto}/*!@.mobile-menu__close-button*/.mobile-menu__close-button.sc-nvb-header-b2c{display:flex;justify-content:center;align-items:center;position:relative;width:44px;height:44px}/*!@.mobile-menu__close-button::after*/.mobile-menu__close-button.sc-nvb-header-b2c::after{content:\" \";display:block;width:24px;height:24px;mask-position:center center;-webkit-mask-position:center center;mask-image:url(\"data:image/svg+xml;base64,\");-webkit-mask-image:url(\"data:image/svg+xml;base64,\");mask-repeat:no-repeat;-webkit-mask-repeat:no-repeat;transform-box:fill-box;transform-origin:center;background-color:#636363}/*!@.mobile-menu__header-button*/.mobile-menu__header-button.sc-nvb-header-b2c{width:100px;display:flex}/*!@.mobile-menu__header-button--close*/.mobile-menu__header-button--close.sc-nvb-header-b2c{justify-content:flex-end}/*!@.mobile-menu__header-button--back*/.mobile-menu__header-button--back.sc-nvb-header-b2c{justify-content:flex-start}/*!@.mobile-menu__back-button*/.mobile-menu__back-button.sc-nvb-header-b2c{text-transform:lowercase;display:flex;justify-content:center;align-items:center;position:relative;height:44px;color:#636363}/*!@.mobile-menu__back-button::before*/.mobile-menu__back-button.sc-nvb-header-b2c::before{content:\" \";display:block;width:24px;height:24px;mask-position:center center;-webkit-mask-position:center center;mask-image:url(\"data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTYiIGhlaWdodD0iMTYiIGZpbGw9Im5vbmUiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PHBhdGggZD0iTTE2IDdIMy44M2w1LjU5LTUuNTlMOCAwIDAgOGw4IDggMS40MS0xLjQxTDMuODMgOUgxNlY3WiIgZmlsbD0iIzMyMzIzMiIvPjwvc3ZnPg==\");-webkit-mask-image:url(\"data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMTYiIGhlaWdodD0iMTYiIGZpbGw9Im5vbmUiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PHBhdGggZD0iTTE2IDdIMy44M2w1LjU5LTUuNTlMOCAwIDAgOGw4IDggMS40MS0xLjQxTDMuODMgOUgxNlY3WiIgZmlsbD0iIzMyMzIzMiIvPjwvc3ZnPg==\");mask-repeat:no-repeat;-webkit-mask-repeat:no-repeat;transform-box:fill-box;transform-origin:center;background-color:#636363;margin-right:0.25rem}/*!@.mobile-menu__header-center-placeholder*/.mobile-menu__header-center-placeholder.sc-nvb-header-b2c{flex:1;display:flex}/*!@.mobile-menu__stack*/.mobile-menu__stack.sc-nvb-header-b2c{flex:1;display:flex;overflow-x:hidden;position:relative}/*!@.mobile-menu__stack-inner*/.mobile-menu__stack-inner.sc-nvb-header-b2c{position:absolute;display:flex;width:300%;min-height:100%;top:0;left:0;transition:transform 0.3s ease}/*!@.mobile-menu__stack-inner--screen2*/.mobile-menu__stack-inner--screen2.sc-nvb-header-b2c{transform:translateX(-33.3333333333%)}/*!@.mobile-menu__stack-inner--screen3*/.mobile-menu__stack-inner--screen3.sc-nvb-header-b2c{transform:translateX(-66.6666666667%)}/*!@.mobile-menu__stack-screen*/.mobile-menu__stack-screen.sc-nvb-header-b2c{flex:1;padding:0.5rem 1rem;position:relative}/*!@.mobile-menu__link-list*/.mobile-menu__link-list.sc-nvb-header-b2c{padding:0;margin:0;list-style:none}/*!@.mobile-menu__link-list li + li*/.mobile-menu__link-list.sc-nvb-header-b2c li.sc-nvb-header-b2c+li.sc-nvb-header-b2c{border-top:1px solid #c9c9c9}/*!@.mobile-menu__link-list a, .mobile-menu__link-list button*/.mobile-menu__link-list.sc-nvb-header-b2c a.sc-nvb-header-b2c,.mobile-menu__link-list.sc-nvb-header-b2c button.sc-nvb-header-b2c{margin-top:0.25rem;margin-bottom:0.25rem;line-height:3rem;display:block;width:100%;text-align:left;color:#636363;text-transform:lowercase;box-sizing:border-box;position:relative}/*!@.mobile-menu__link-list.mobile-menu__link-list--capitalize a,\n.mobile-menu__link-list.mobile-menu__link-list--capitalize button*/.mobile-menu__link-list.mobile-menu__link-list--capitalize.sc-nvb-header-b2c a.sc-nvb-header-b2c,.mobile-menu__link-list.mobile-menu__link-list--capitalize.sc-nvb-header-b2c button.sc-nvb-header-b2c{text-transform:capitalize}/*!@.mobile-menu__link-list button*/.mobile-menu__link-list.sc-nvb-header-b2c button.sc-nvb-header-b2c{padding-right:24px}/*!@.mobile-menu__link-list button::after,\n.mobile-menu__bottom-link::after*/.mobile-menu__link-list.sc-nvb-header-b2c button.sc-nvb-header-b2c::after,.mobile-menu__bottom-link.sc-nvb-header-b2c::after{display:block;width:24px;height:24px;mask-position:center center;-webkit-mask-position:center center;mask-image:url(\"data:image/svg+xml;base64,\");-webkit-mask-image:url(\"data:image/svg+xml;base64,\");mask-repeat:no-repeat;-webkit-mask-repeat:no-repeat;transform-box:fill-box;transform-origin:center;background-color:#636363;content:\" \";position:absolute;right:0;top:50%;margin-top:-12px}/*!@.mobile-menu__link-icon*/.mobile-menu__link-icon.sc-nvb-header-b2c{fill:#32d2ff;display:block;width:24px;height:24px;transform-box:fill-box;transform-origin:center;content:\" \";position:absolute;left:0;top:50%;margin-top:-12px}/*!@.mobile-menu__stack-screen .mobile-menu__link-list:last-of-type*/.mobile-menu__stack-screen.sc-nvb-header-b2c .mobile-menu__link-list.sc-nvb-header-b2c:last-of-type{padding-bottom:3rem}/*!@.mobile-menu__link-list .mobile-menu__link--with-icon*/.mobile-menu__link-list.sc-nvb-header-b2c .mobile-menu__link--with-icon.sc-nvb-header-b2c{padding-left:calc(24px + 0.5rem)}/*!@.mobile-menu__link-list .mobile-menu__link--no-icon*/.mobile-menu__link-list.sc-nvb-header-b2c .mobile-menu__link--no-icon.sc-nvb-header-b2c{padding-left:0.5rem}/*!@.mobile-menu__link-list-title*/.mobile-menu__link-list-title.sc-nvb-header-b2c{font-weight:bold;text-transform:lowercase;margin-top:1rem}/*!@.mobile-menu__bottom-link*/.mobile-menu__bottom-link.sc-nvb-header-b2c{display:block;line-height:3rem;background:#f6f6f6;width:100%;color:#000;box-sizing:border-box;padding-left:1rem;position:absolute;left:0;bottom:0;text-transform:lowercase}/*!@.mobile-menu__bottom-link::after*/.mobile-menu__bottom-link.sc-nvb-header-b2c::after{right:1rem}";
...
class NvbHeaderB2c {
  ...
  render() {
    ...
    return (hAsync("header", null, hAsync("div", { class: "container container--top" }, hAsync("div", { class: 'top' }, hAsync("a", Object.assign({ href: "/werkgever" }, AnalyticsDataAttributes$1({ action: 'ClickOnHeaderWerkgeversLink', label: 'VoorWerkgevers' })), "Werkgevers / Plaats vacature"))), hAsync("nav", { class: 'main' }, hAsync("div", { class: "container" }, hAsync("div", { class: "main-inner" }, hAsync("div", { class: "mobile-menu-button-container" }, hAsync("button", { class: "mobile-menu-button", onClick: () => this.handleOpenMenu(), "aria-label": "Open menu" }, hAsync(Icons.menu, { className: 'mobile-menu-button__icon' }))), hAsync(Logo, null), hAsync("div", { class: 'menu' }, hAsync("div", { class: dropdownActiveClassExtender('/vacature') }, hAsync("button", { class: 'menu-link menu-link--dropdown' }, "Vacatures"), hAsync(DropdownContainerJobs, null)), hAsync("div", { class: dropdownActiveClassExtender('/carriere') }, hAsync("button", { class: 'menu-link menu-link--dropdown' }, "Carri\u00E8retips"), hAsync(DropdownContainerCareer, null)), hAsync("a", Object.assign({ href: "/salariswijzer", class: linkActiveClassExtender('/salariswijzer') }, AnalyticsDataAttributes$1({
      action: 'ClickOnHeaderWerkZoekendenLink',
      label: 'Salariswijzer'
    })), "Salariswijzer"), hAsync("hr", { class: 'menu-separator' }), hAsync("div", { class: dropdownActiveClassExtender(['/account', '/profiel']) }, hAsync("button", { class: 'menu-link menu-link--dropdown' }, "Account"), this.isAuthenticated && hAsync(DropdownContainerAccountAuthenticated$1, null), !this.isAuthenticated && hAsync(DropdownContainerAccountAuthenticated, null)), hAsync("div", { class: "dropdown-backdrop" })), hAsync("div", { class: 'account-button-container' }, hAsync(AccountButton, null))))), hAsync(MobileMenu, { isAuthenticated: this.isAuthenticated, isVisible: this.isActiveMobileMenu, handleModalClose: () => this.handleClose(), activeMobileMenuLevel: this.activeMobileMenuLevel, setActiveMobileMenuLevel: level => this.setActiveMobileMenuLevel(level) })));
  }
  static get watchers() { return {
    "isActiveMobileMenu": ["adjustDocument"]
  }; }
