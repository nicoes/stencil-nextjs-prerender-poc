...
const AnchorList$1 = ({ listItems, analyticsAction, className }) => listItems.map(item => hAsync("li", null,
  hAsync("a", Object.assign({ class: className || '' }, AnalyticsDataAttributes({ action: analyticsAction, label: item.analyticsLabel || item.label }), { href: item.value }), item.label)));
...
const MainMenuLink = ({ item }) => hAsync("a", Object.assign({ href: item.value }, AnalyticsDataAttributes({ action: 'ClickOnHeaderLink', label: item.analyticsLabel || item.label }), { class: 'menu-link menu-link--dropdown' }), item.label);
...
class IolHeaderB2c {
  ...
  render() {
    ...
    return (hAsync("header", null, hAsync("div", { class: "container container--top" }, hAsync(TopBar, null)), hAsync("nav", { class: 'main' }, hAsync("div", { class: "container" }, hAsync("div", { class: "main-inner" }, hAsync("div", { class: "mobile-menu-button-container" }, hAsync("button", { class: "mobile-menu-button", onClick: () => this.handleOpenMenu(), "aria-label": "Open menu" }, hAsync(Icons$1.menu, { className: 'mobile-menu__header-icon' }))), hAsync("div", { class: 'menu' }, hAsync("div", { class: dropdownActiveClassExtender('/vacature') }, hAsync(MainMenuLink, { item: mainMenuLinks.VACATURES }), hAsync(DropdownContainer, null)), hAsync("div", { class: dropdownActiveClassExtender(topicsListItems.map(item => item.value)) }, hAsync(MainMenuLink, { item: mainMenuLinks.TOPICS }), hAsync(DropdownContainer$1, { listItems: topicsListItems, analyticsAction: 'ClickOnHeaderTopicsLink' }))))))));
  }
